...
function deleteAllTeamData(teamId) {
    const db = getDb();
    const boardRows = db.prepare('SELECT id FROM boards WHERE team_id = ?').all(teamId);
    for (const b of boardRows) {
        const resources = db.prepare('SELECT url, original_url FROM resources WHERE board_id = ?').all(b.id);
...
router.post('/teams', (req, res) => {
    ...
    db.prepare('INSERT INTO teams (id, name, icon, invite_code, owner_id, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(id, req.body.name, req.body.icon || '🎬', inviteCode, user.id, now);
    db.prepare('INSERT INTO team_members (team_id, user_id, name, email, role, joined_at) VALUES (?, ?, ?, ?, ?, ?)').run(id, user.id, user.name, user.email, 'owner', now);
    res.json({
        team: {
            id,
            name: req.body.name,
...
router.get('/teams/:id', (req, res) => {
    ...
    res.json({
        team: {
            ...
            memberCount: members.length,
            members: members.map((m) => ({ userId: m.user_id, name: m.name, email: m.email, role: m.role, joinedAt: m.joined_at })),
...
router.put('/teams/:id', (req, res) => {
    ...
    if (req.body.icon) {
        if (team.icon && team.icon !== req.body.icon)
            deleteUpload(team.icon);
...
router.patch('/teams', (req, res) => {
    const db = getDb();
    const team = db.prepare('SELECT * FROM teams WHERE id = ?').get(req.body.teamId);
    if (!team) {
        res.status(404).json({ error: '团队不存在' });
        return;
    }
...
router.get('/teams/lookup', (req, res) => {
    ...
    const db = getDb();
    const team = db.prepare('SELECT * FROM teams WHERE invite_code = ?').get(code.trim().toUpperCase());
    if (!team) {
        res.status(404).json({ error: '邀请码无效或团队不存在' });
        return;
    }
    const owner = db.prepare('SELECT name FROM users WHERE id = ?').get(team.owner_id);
    const members = db.prepare('SELECT * FROM team_members WHERE team_id = ?').all(team.id);
    const user = req.user;
...
router.get('/teams/join', (req, res) => {
    const db = getDb();
    const members = db.prepare('SELECT * FROM team_members WHERE team_id = ?').all(req.query.teamId);
    res.json({ members: members.map((m) => ({ userId: m.user_id, name: m.name, email: m.email, role: m.role, joinedAt: m.joined_at })) });
});
router.post('/teams/join', (req, res) => {
    ...
    if (!tid && req.body.inviteCode) {
        const team = db.prepare('SELECT id FROM teams WHERE invite_code = ?').get(req.body.inviteCode);
        if (!team) {
    ...
    db.prepare('INSERT INTO team_members (team_id, user_id, name, email, role, joined_at) VALUES (?, ?, ?, ?, ?, ?)').run(tid, user.id, user.name, user.email, 'editor', new Date().toISOString());
    const team = db.prepare('SELECT * FROM teams WHERE id = ?').get(tid);
    res.json({ team: { id: team.id, name: team.name, icon: team.icon, inviteCode: team.invite_code, ownerId: team.owner_id, createdAt: team.created_at } });
});
router.patch('/teams/join', (req, res) => {
    const db = getDb();
...
router.delete('/teams/join', (req, res) => {
    const db = getDb();
