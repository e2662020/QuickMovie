...
interface Room {
  id: string
  documentId: string
  clients: Map<string, { ws: WebSocket; userId: string; userName: string; color: string }>
}
...
function handleJoin(ws: WebSocket, msg: any, setIds: (uid: string, rid: string) => void) {
...
function handleEdit(roomId: string, userId: string, msg: any) {
  broadcast(roomId, {
    type: 'edit',
    userId,
...
function handleCursor(roomId: string, userId: string, msg: any) {
  broadcast(roomId, {
    type: 'cursor',
    userId,
