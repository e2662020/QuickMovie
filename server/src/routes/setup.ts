...
router.get('/setup/config', (_req, res) => {
  const db = getDb()
  const rows = db.prepare("SELECT key, value FROM settings WHERE key != 'installed'").all() as { key: string; value: string }[]
...
router.post('/setup/reset', (req, res) => {
  ...
  const db = getDb()
  const adminEmailRow = db.prepare("SELECT value FROM settings WHERE key = 'admin_email'").get() as { value: string } | undefined
  if (!adminEmailRow) {
    res.status(400).json({ error: '系统尚未安装' })
    return
  }
  const adminUser = db.prepare('SELECT password_hash FROM users WHERE email = ?').get(adminEmailRow.value) as { password_hash: string } | undefined
