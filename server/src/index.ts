...
function deleteUploadFile(url: string | null): void {
...
v1.get('/boards', (req, res) => {
  ...
  if (!teamId) { res.status(400).json({ error: '缺少 teamId 参数' }); return }
  const boards = db.prepare('SELECT * FROM boards WHERE team_id = ?').all(teamId) as any[]
...
v1.get('/files', (req, res) => {
  ...
  if (!boardId) { res.status(400).json({ error: '缺少 boardId 参数' }); return }
  let rows: any[]
  if (req.query.parentId) {
    rows = db.prepare('SELECT * FROM board_files WHERE board_id = ? AND parent_id = ? ORDER BY sort_order').all(boardId, req.query.parentId as string)
  } else {
...
v1.post('/files', (req, res) => {
  ...
  if (!boardId || !name) { res.status(400).json({ error: '缺少必要参数' }); return }
  const now = new Date().toISOString()
  const id = 'file-' + uid()
  const existing = db.prepare('SELECT COUNT(*) as cnt FROM board_files WHERE board_id = ?').get(boardId) as { cnt: number }
  db.prepare('INSERT INTO board_files (id, board_id, parent_id, name, type, content, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
    id, boardId, parentId || null, name, type || 'note', content || '', existing.cnt, now, now
  )
...
v1.delete('/files', (req, res) => {
  ...
  if (!fileId) { res.status(400).json({ error: '缺少 fileId 参数' }); return }
  ...
  while (queue.length > 0) {
    ...
    for (const child of children) { idsToDelete.push(child.id); queue.push(child.id) }
  }
  for (const id of idsToDelete) {
    ...
    for (const r of resources) {
      ...
      if (r.original_url) deleteUploadFile(r.original_url)
    }
    db.prepare('DELETE FROM resources WHERE file_id = ?').run(id)
    db.prepare('DELETE FROM story_elements WHERE file_id = ?').run(id)
  }
  for (const id of idsToDelete) { db.prepare('DELETE FROM board_files WHERE id = ?').run(id) }
  res.json({})
})
...
v1.get('/elements', (req, res) => {
  ...
  const boardId = req.query.boardId as string
  if (!boardId) { res.status(400).json({ error: '缺少 boardId 参数' }); return }
  const elements = db.prepare('SELECT * FROM story_elements WHERE board_id = ?').all(boardId) as any[]
...
v1.post('/elements', (req, res) => {
  ...
  if (!boardId || !type || !name) { res.status(400).json({ error: '缺少必要参数' }); return }
  const now = new Date().toISOString()
  const id = 'elem-' + uid()
  db.prepare('INSERT INTO story_elements (id, board_id, file_id, type, name, content, color, position, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
    id, boardId, fileId || null, type, name, content || '', color || '#6b7280', position || null, now, now
  )
...
v1.delete('/elements', (req, res) => {
  ...
  if (!elementId) { res.status(400).json({ error: '缺少 elementId 参数' }); return }
...
v1.get('/resources', (req, res) => {
  ...
  const boardId = req.query.boardId as string
  if (!boardId) { res.status(400).json({ error: '缺少 boardId 参数' }); return }
  const resources = db.prepare('SELECT * FROM resources WHERE board_id = ?').all(boardId) as any[]
...
v1.delete('/resources', (req, res) => {
  ...
  if (!resourceId) { res.status(400).json({ error: '缺少 resourceId 参数' }); return }
  const r = db.prepare('SELECT url, original_url FROM resources WHERE id = ?').get(resourceId) as any
  if (r) {
    deleteUploadFile(r.url)
    if (r.original_url) deleteUploadFile(r.original_url)
  }
